...
fixes). Human-facing description: `macos/README.md`. Engineering record:
`macos/PORTING.md` (what was built and why), `macos/GAPS.md` (audit: what differs
from Windows), `macos/PLAN.md` (the work plan, in Russian), `macos/FEATURES.md`
(generated menu-command coverage).
...
3. **Docs move with the code**: `GAPS.md` for anything that changes the comparison with Windows,
   `PORTING.md` for design and numbers, `implemented.txt` / `FEATURES.md` (generated by
...
The translations are model-made and unreviewed by native speakers (noted in `GAPS.md`).
...
- `PLAN.md` 0.3: Developer ID signing and notarisation of the `.dmg` (`package.sh` has the hooks;
  never yet run with real credentials - expect hardened-runtime / entitlement fixes), then
  `spctl --assess` on a clean machine. Credentials belong in the keychain
  (`xcrun notarytool store-credentials`), never in the repository, the environment of CI logs, or a chat.
- Review of the model-made translations by native speakers.
- Whatever `GAPS.md` lists under "What the Windows version has and this does not".
